...
const SendOTP = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setLoading(true);
        const code = e.target.elements.countryCode.value;
        const phone = e.target.elements.phone.value;
        if (code && phone) {
            await axios.post(`http://localhost:5000/api/users/phone-verification`, { id: user.id, code, phone })
                .then(res => {
                    if (res.status === 200) {
                        navigate(`/verify-phone/${user.id}&&${code + phone}`);
                    }
                })
                .catch(err => {
                    console.log(err);
            ...
            <div className="bg-white rounded shadow-sm mx-auto" style={{ padding: 30, maxWidth: 600, width: "100%" }}>
                ...
                <form className="form w-100" onSubmit={handleSubmit}>
                    <div className="text-center" style={{ marginBottom: 20 }}>
                        <h4 className="text-dark mb-3">Verify Mobile Number</h4>
                    </div>
                    <div className="fv-row" style={{ marginBottom: 20 }}>
                        <label className="form-label fs-6 fw-bolder text-dark">Select country</label>
                        <select className="form-control" name="countryCode">
                            ...
                            <option data-countryCode="PR" value="+92">Pakistan (+92)</option>
                    ...
                    <div className="fv-row" style={{ marginBottom: 20 }}>
                        ...
                        <input className="form-control form-control-lg" type="number" name="phone" autoComplete="off" />
                        <span className="fv-help-block text-dark">
                            <span className="text-dark">Enter your mobile number without country code.</span>
                        </span>
                    </div>
